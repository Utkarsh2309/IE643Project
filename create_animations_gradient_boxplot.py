...
def load_histogram_images(block, component, epoch_path):
    ...
    for batch_folder in batch_folders:
        gradient_image_path = os.path.join(batch_folder, "gradients_boxplot.png")  # Assuming gradient histogram is saved with this name
    ...
    for block in [3, 7]:  # Block 4 and Block 8
        for component in ["attention", "mlp"]:
            ...
            for epoch in epoch_folders:
                ...
                save_path = os.path.join(animation_dir, f"gradients_boxplot_block_{block}_{component}_epoch_{epoch}.gif")
                
                # Create animation if images are found
                if images:
                    create_animation(images, save_path)
                else:
                    print(f"[WARNING] Skipping animation for gradient boxplots in Block {block}, Component '{component}', Epoch {epoch} due to missing images.")

# Define directories for plots and animations
plot_dir = "./plots_gradients_boxplots"
